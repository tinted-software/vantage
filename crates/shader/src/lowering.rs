...
///
/// `expf_fid` is the module-assigned `FuncId` (as u32) of an *import*
/// declaration for `@vantage_expf`, required whenever the IR contains calls
/// (exponential fog). The FuncRef is built exactly like
/// `Module::declare_func_in_func` so the JIT resolves the relocation through
/// the module declaration (namespace 0 = function index); `ExternalName::TestCase`
/// is rejected by `ModuleReloc::from_mach_reloc` (`unimplemented!()`).
pub fn lower_to_clif(ir: &FragmentIr, expf_fid: Option<u32>) -> Function {
    let ctx = &*ir.ctx;
    ...
    // Module-resolvable FuncRef for @vantage_expf when the IR calls it.
    let expf_fref: Option<cranelift_codegen::ir::FuncRef> = expf_fid.map(|idx| {
        let sig = Signature {
            params: alloc::vec![AbiParam::new(types::F32)],
            returns: alloc::vec![AbiParam::new(types::F32)],
            call_conv: CallConv::SystemV,
        };
        let sigref = b.func.import_signature(sig);
        let uname =
            b.func
        ...
        b.func.import_function(cranelift_codegen::ir::ExtFuncData {
            name: cranelift_codegen::ir::ExternalName::user(uname),
            signature: sigref,
            colocated: false,
            patchable: false,
        })
        ...
        for op_ptr in blk_ptr.deref(ctx).iter(ctx) {
            ...
            match op_name.as_str() {
                ...
                "llvm.bitcast" => {
                    ...
                    let res =
                        b.ins()
                            .bitcast(res_ty, cranelift_codegen::ir::MemFlagsData::new(), v);
                    val_map.insert(op.get_result(0), res);
                }
                "llvm.br" => {
                ...
                "llvm.call" => {
                    // Direct call to an external symbol; the only callee the
                    // fragment builder emits is @vantage_expf (exponential fog).
                    let cop = crate::lowering::wrap::<CallOp>(op_ptr, ctx);
                    let callee = CallOpInterface::callee(&cop, ctx);
                    let CallOpCallable::Direct(ident) = callee else {
                        panic!("indirect calls unsupported in fragment lowering");
                    };
                    let callee_name = alloc::string::ToString::to_string(&ident);
                    assert_eq!(
                        callee_name.as_str(),
                        crate::program::EXPF_NAME,
                        "unexpected callee {callee_name}"
                    );
                    let fref = expf_fref.expect("expf FuncRef prepared for call IR");
                    let arg = val_map[&op.get_operand(0)];
                    let call_inst = b.ins().call(fref, &[arg]);
                    let res = b.inst_results(call_inst)[0];
                    let res_val = op.get_result(0);
                    val_map.insert(res_val, res);
                }
