...
use pliron_llvm::op_interfaces::NNegFlag;
use pliron_llvm::op_interfaces::{
    BinArithOp, CastOpInterface, FastMathFlags, IntBinArithOp, IntBinArithOpWithOverflowFlag,
};
use pliron_llvm::ops::{
...
pub const EXPF_NAME: &str = "vantage_expf";
...
impl<'c> B<'c> {
    ...
    fn at_block(&mut self, blk: Ptr<pliron::basic_block::BasicBlock>) {
        ...
        // Constants are emitted where they are used: a value defined in one
        // block and used in a non-descendant (e.g. shared between sibling
        // near/linear filter blocks) violates SSA dominance in the
        // cranelift verifier. Dedup stays within the current block only.
        self.c_f32.clear();
        self.c_i64.clear();
        self.c_i8.clear();
        self.c_i32.clear();
    ...
    fn ibin<T: IntBinArithOp + IntBinArithOpWithOverflowFlag>(
        &mut self,
        a: Value,
        b: Value,
    ) -> Value {
        emit!(self, |ctx: &mut Context| T::new_with_overflow_flag(
            ctx,
            a,
            b,
    ...
    fn uitofp32(&mut self, v: Value) -> Value {
        let t = f32t(self.ctx);
        emit!(self, |ctx: &mut Context| {
            let op = UIToFPOp::new(ctx, v, t);
            NNegFlag::set_nneg(&op, ctx, false);
            op
        })
    }
...
pub fn build_fragment_program(key: &FragmentKey) -> FragmentIr {
    ...
    // NOTE: `mregion` (the region helper blocks attach to) is bound below
    // once `entry` exists — it must be the *function's* region, not the
    // module's; blocks appended to the module region are invisible to
    // lowering, which walks `func.get_region(0)`.
    ...
    let mregion = entry
        .deref(&ctx)
        .get_parent_region()
        .expect("entry block in func region");
    ...
    for unit in 0..2usize {
        ...
        fn fetch(b: &mut B<'_>, t: &TexCtx, x: Value, y: Value) -> [Value; 4] {
            ...
            let ok_blk = b.block(vec![], t.mregion);
            let tf = b.t_f32();
            ...
            // Normalize to f32 in the ok path so both predecessors of the
            // join deliver f32 (the bad path yields literal [0,0,0,1]).
            let s255 = b.f(255.0);
            let r_f = b.uitofp32(a0);
            let r = b.fdiv(r_f, s255);
            let g_f = b.uitofp32(a1);
            let g = b.fdiv(g_f, s255);
            let b_f = b.uitofp32(a2);
            let bb = b.fdiv(b_f, s255);
            let a_f = b.uitofp32(a3);
            let aa = b.fdiv(a_f, s255);
            b.br(ret_blk, vec![r, g, bb, aa]);
            b.at_block(ret_blk);
            let rargs: Vec<Value> = ret_blk.deref(&*b.cx()).arguments().collect();
            [rargs[0], rargs[1], rargs[2], rargs[3]]
        };
        ...
        let tf = b.t_f32();
        let near_blk = b.block(vec![], mregion);
        let lin_blk = b.block(vec![], mregion);
...
impl FragmentIr {
    /// True if the program contains an `llvm.call` (e.g. `@vantage_expf` for
    /// exponential fog). The cranelift-interpreter backend cannot execute
    /// host symbol calls, so those programs fall back to scalar evaluation.
    pub fn has_external_calls(&self) -> bool {
        use pliron::linked_list::ContainsLinkedList;
        let c: &Context = &self.ctx;
        for region in self.func.deref(c).regions() {
            for blk in region.deref(c).iter(c) {
                for op in blk.deref(c).iter(c) {
                    if alloc::string::ToString::to_string(&Operation::get_opid(op, c).disp(c))
                        .as_str()
                        == "llvm.call"
                    {
                        return true;
...
/// Verify the function round-trips through the LLVM text parser
/// (catches malformed attributes/wrong operand kinds).
///
/// `llvm.func` is `IsolatedFromAbove`, so the whole function dumps and
/// re-parses as one top-level op with its own SSA name scope. Parsing
/// individual instruction lines at top level would panic in pliron's
/// `NameTracker` (no active scope outside an isolated region).
fn verify_all(ctx: &mut Context, fop: Ptr<Operation>) {
    ...
    // Per-op verify first so failures name the offending operation.
    {
        use pliron::common_traits::Verify as _V;
        use pliron::linked_list::ContainsLinkedList as _CL;
        let c: &Context = ctx;
        for region in fop.deref(c).regions() {
            for blk in region.deref(c).iter(c) {
                for op in blk.deref(c).iter(c) {
                    op.deref(c).verify(c).unwrap_or_else(|e| {
                        panic!("verify failed for {}: {e}", op.deref(c).disp(c))
                    });
                }
            }
        }
    ...
    let dump = {
        let c: &Context = ctx;
        alloc::string::ToString::to_string(&fop.deref(c).disp(c))
    };
    let reparsed = parse_from_str(spaced(OpStruct::top_level_parser()), ctx, dump.trim())
        .unwrap_or_else(|e| panic!("func round-trip parse failed: {e}\n--- dump ---\n{dump}"));
    let c: &Context = ctx;
    reparsed
        .deref(c)
        .verify(c)
        .unwrap_or_else(|e| panic!("func verify failed: {e}\n--- dump ---\n{dump}"));
}
